...
mod tests {
    ...
    fn correct_operation_called_mul() {
        ...
        cpu.decode(0xE01_432_91).unwrap().execute(&mut cpu, &mut bus); // MULS R4, R2, R1 (R4 = 20 * 0xFFFFFFF6u32)
    ...
    fn correct_operation_called_mla() {
        ...
        cpu.set_register(2, 0x00000014); // Rs
        cpu.set_register(1, 0xFFFFFFF6); // Rm
        cpu.decode(0xE02_432_91).unwrap().execute(&mut cpu, &mut bus); // MLA R4, R2, R1, R3 (R4 = 20 * 0xFFFFFFF6u32 + 1)
    ...
    fn correct_operation_called_umull() {
        ...
        cpu.set_register(2, 0x00000014); // Rs
        cpu.set_register(1, 0xFFFFFFF6); // Rm
        cpu.decode(0xE09_432_91).unwrap().execute(&mut cpu, &mut bus); // UMULLS  R3, R4, R2, R1 (R4,R3 = 20 * 0xFFFFFFF6u32)
    ...
    fn correct_operation_called_umlal() {
        ...
        cpu.set_register(2, 0x00000014); // Rs
        cpu.set_register(1, 0xFFFFFFF6); // Rm
        cpu.decode(0xE0A_432_91).unwrap().execute(&mut cpu, &mut bus); // UMLAL  R3, R4, R2, R1 (R4,R3 = 20 * 0xFFFFFFF6u32 + 0x1,0x1)
    ...
    fn correct_operation_called_smull() {
        ...
        cpu.set_register(2, 0x00000014); // Rs
        cpu.set_register(1, 0xFFFFFFF6); // Rm
        cpu.decode(0xE0C_432_91).unwrap().execute(&mut cpu, &mut bus); // SMULL  R3, R4, R2, R1 (R4,R3 = 20 * -10i32)
        assert_eq!(cpu.get_register(3), 0xFFFFFF38);
    ...
    fn correct_operation_called_smlal() {
        ...
        cpu.set_register(2, 0x00000014); // Rs
        cpu.set_register(1, 0xFFFFFFF6); // Rm
        cpu.decode(0xE0F_432_91).unwrap().execute(&mut cpu, &mut bus); // SMLALS  R3, R4, R2, R1 (R4,R3 = 20 * -10i32 + 0x0,0xC8 [should add 200 to -200 to equal 0])
